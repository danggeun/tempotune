...
const SPEC = {
  ...
  gap: 0.053,         // 잉크 하단 ↔ 띠 상단. 3.8 % 는 앞선 심사가 준 '하한'(밑줄 거리 탈출선)을 그대로 쓴 값이라 이탤릭 'G' 꼬리 밑이 눌렸다.
                      // 5.3 % 에서 꼬리 밑 실측이 5.4 % 가 되어 압박이 풀리고, 띠 아래 여백이 간격보다 커서 한 덩어리 결속은 유지된다 (6 % 이상이면 두 물체로 분리)

...
function runFile(adapterFactory, wavPath) {
  ...
  for (const f of frames) {
    ...
    if (s.midi != null || s.gliss) {
      ...
      if (f.t - s.t0 >= STEADY_SKIP) {
        if (shown) {
          const c = 1200 * Math.log2(f.hz / eh)
          const m = hzToMidi(f.hz), em = hzToMidi(eh)
          octaveErr.push(Math.abs(m - em) === 12 ? 1 : 0)
          if (Math.abs(m - em) !== 12) centsErr.push(c)
        } else { centsErr.push(NaN) } // 정상 상태인데 표시 없음 → 누락
      }
  ...
  return {
    ...
    centsBias: median(ce), centsP90: pct(ce.map(Math.abs), .9), missingPct: centsErr.length ? 100 * missing / centsErr.length : NaN,

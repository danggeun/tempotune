...
### 수정 — 튜너
- **중음(더블스톱)에서 연주하지 않은 낮은 음을 표시하던 문제.** 두 현을 함께 켜면 YIN 이 두 음의 최대공약수(가상 기본음)를 높은 신뢰도로 내서, 바이올린이 낼 수 없는 음(G1 등)이 신뢰도 0.5 이상 프레임의 **23.7 %** 를 차지했다. 가상 기본음의 정수배 자리를 조사해 실제로 울리는 음(위 성부)으로 되돌린다. 두 음이 울리는 동안 바늘이 오가지 않도록 직전 음을 붙잡고, 약한 기본음 단음(첼로 C2·베이스 E1)을 중음으로 오인하지 않도록 정합성 거부권을 둔다.
- 스펙트럼에 에너지가 없는 음은 표시 경로에서 배제 (연주 감지에는 원본 신뢰도 유지 — 중음 구간 연습시간이 빠지지 않게).
- 실측(v2.0.0 → 2.0.1, 같은 녹음): 가짜음 23.96 % → **4.29 %** · 중음 구간 55.69 % → **8.87 %** · 중음 흔들림 16.44 % → **10.82 %** · 표시율 93.9 % → 92.4 % · 연주 감지 19,292 프레임 중 다른 것 0개.
...
### 검증
- 단위 99 (+26) · e2e 38 · 스크린샷 12장 0px · 합성 신호 40개 중 39개 무변화(1개 개선) · 합성 중음 12종 12/12 · 기본음 −60 dB 단음 유지 · 독립 DSP 리뷰 1회(회귀 1건 지적 → 수정)

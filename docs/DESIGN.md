...
**Phase 2 구현 결과 (2026-09-05)** — 벤치마크로 조정된 최종값과 이유:
- 중앙값 창 5→3, conf 0.6→0.5, 전환 3→2프레임: 스케일 락 지연 124→101 ms. 5프레임은 음 전환에 2프레임 추가 지연을 줬다.
- 표시값은 항상 중앙값을 따라가고 라벨만 히스테리시스: 글리산도에서 바늘이 끊기지 않는다.
- 적응 평활은 오차가 **같은 방향으로 4프레임 이상** 벌어질 때만 켠다 — 비브라토(반주기 3–4프레임마다 부호 반전)는 설정 계수로 평활된 중심을 유지 (리뷰 지적).
- 유효 프레임 끊김 유지 2→6프레임(≈140 ms): 활 바꿈·현 이동에서 표시가 깜빡이지 않게.
- 옥타브 교정은 스펙트럼 **존재+레벨** 조건: f0/2 와 3f0/2 가 f0 피크 대비 −15/−20 dB 이내일 때만 절반으로(공명 개방현 −20 dB 로는 안 떨어짐), f0 부재·2f0·4f0 존재·**3f0 부재**일 때만 두 배로(기본음이 약한 저음현은 3f0 가 있어 유지).
...
**Phase 2 구현 결과 (2026-09-05)** — 정직한 트레이드오프:
- 잡음·무음·클릭은 0% 오검출 (v1: 음 끝난 뒤 1 s 유지로 25%). 프레임당 2 ms.
- **대화 음절(150–300 ms)과 스타카토 음(150 ms)은 물리적으로 구분되지 않는다.** attack 을 325 ms 로 두면 합성 말소리 0% 이지만 스타카토·피치카토 런이 시작조차 못 한다. 연습 앱은 음악 쪽을 택한다: 기본 attack 140 ms(+감점식 카운터, 250 ms 유지) → 스타카토 런 F1 0.7, 대화는 ≈90% 연주로 센다(v1 99%). 엄격 프리셋(`STRICT_DETECTOR`)은 코드에 남겨 설정 노출 여부를 실사용 후 결정.
- f0 안정도는 "연속 구간 회귀 드리프트 > 60¢" 로 구현 — 급한 억양(≥ 500¢/s)만 거른다. 비브라토는 왕복이라 영향 없음.

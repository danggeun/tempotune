...
- [A] 메트로놈 재생 중 펼쳐서 박자 변경, 사용자가 접은 상태는 정지 후에도 유지, 시작 시 자동 접힘은 정지 시 복귀
- [A] 앱 첫 실행 권한 안내 팝업 1회 → 거부 시 '설정 › 앱 › 권한' 안내 (웹은 사이트 설정) — [D] 실기기에서 두 번 거부 후 복구
- [D] Android 15 엣지투엣지에서 헤더가 상태바와 겹치지 않음, 런처 아이콘(adaptive) 이 검정 무대 + 'Go' 로 보임, 앱 이름 'Go practice'
- [D] Windows 에서 `npm run cap:sync` (cross-env) → Android Studio 서명 APK → 설치 → 버전 2.0.0 표시

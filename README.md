...
**앱 완결성** — 오프라인 PWA(프리캐시, 자체 호스팅 폰트). 새 버전은 유휴일 때 적용하고 아니면 알린다. 인터럽트 복구, 유휴 시 컨텍스트 suspend, Wake Lock. 화면이 숨겨지면 마이크를 놓고 메트로놈을 멈춘다. 녹음은 10초마다 저장돼 앱이 죽어도 복구된다. 다크 고정, 터치 타겟 44 px, 대비 WCAG AA.

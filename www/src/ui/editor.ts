...
function loopTick(): void {
  ...
  if (ed.loop && ed.ptA !== null && ed.ptB !== null && a.currentTime >= ed.ptB - 0.02) a.currentTime = loopStart()
...
function checkExportBtn(): void { q('ed-export-btn').classList.toggle('dim', ed.ptA === null || ed.ptB === null) }
...
export function openEditor(item: RecItem): void {
  ...
  const restore = () => {
    ...
    if (item.ab && item.ab.b > item.ab.a) { ed.ptA = item.ab.a; ed.ptB = Math.min(item.ab.b, dur()); updateHandles(); updateABtn(); updateBBtn(); updateLoopBtn(); checkExportBtn() }
...
export function closeEditor(): void {
  ...
  removeWindowHandlers()
  showMenuInstant()
  const page = q('editor-page'); page.classList.remove('open'); page.style.display = 'none'
}
/** 삭제되는 항목을 편집 중이면 닫는다 */
...
function setSpeed(v: number, persist = true): void {
  ...
  q<HTMLInputElement>('ed-speed').value = String(v)
  if (ed.audio) { ed.audio.playbackRate = v; ed.audio.preservesPitch = true }
  if (persist && ed.item && ed.item.speed !== v) { const next = patchRec(ed.item, { speed: v }); if (next) ed.item = next }
}
/** 값 라벨 탭: 1.0 → 0.5 → 0.7 → 0.85 → 1.0 순환 (활 든 손으로 슬라이더 대신 한 번씩 탭) */
function cycleSpeed(): void {
  const cur = ed.audio?.playbackRate ?? 1
  const i = SPEED_PRESETS.findIndex(p => Math.abs(p - cur) < 0.01)
  setSpeed(SPEED_PRESETS[(i + 1) % SPEED_PRESETS.length]!)
}
...
function renderBmTicks(): void {
  ...
  for (const t of ed.bookmarks) {
    ...
    tick.className = 'bm-tick'; tick.style.left = pct(t, d)

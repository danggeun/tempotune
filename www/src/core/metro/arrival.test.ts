...
// 순수 추정기
describe('arrival: candidate agreement rules', () => {
  ...
  test('0 before agreement — two candidates aren’t enough', () => {
    const a = createArrival(); feed(a, 1, 1.1); feed(a, 2, 2.1)
    expect(a.offset()).toBe(0); expect(a.diag().applied).toBe(false)
  })
  test('applies the median once minAgree candidates gather within ±agreeTol', () => {
  ...
  test('missed beats (while playing, with headphones) don’t push the correction', () => {
  ...
  test('scattered candidates (false spikes) aren’t applied', () => {
  ...
  test('drops candidates out of range (over 300 ms)', () => {
  ...
  test('spikes below minProminence × median aren’t candidates', () => {
...
describe('self-calibrating metronome click arrival (same flow as the app)', () => {
  ...
  test('device with zero offset: the correction stays near 0 and nothing changes', () => {
  ...
  test('headphones (clicks don’t reach the mic): no correction', () => {
...
describe('pending cap', () => {
  test('never exceeds 64 even without update', () => {
    const a = createArrival()

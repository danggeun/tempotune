/** 기준음 재생 (오실레이터). 단일 컨텍스트라 마이크 없이도 소리가 난다 */
...
/** 튜너 헤더의 'A 듣기': 기준음(refHz) 그대로의 A4 를 토글 */
...
export function adjRefOctave(d: number): void {
  ...
  if (active && active !== 'A4') { stopRefNote(); toggleRefNote(active) } // 재생 중이면 새 옥타브로 즉시 갱신 (A4 는 옥타브 무관)
}

onMic('beforeClose', stopRefNote) // 마이크를 끄면 기준음도 정지

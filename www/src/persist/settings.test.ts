...
describe('parseStored', () => {
  ...
  test('v1 format migrates (TTL 무시)', () => {
    ...
    expect(parseStored(JSON.stringify(v1))).toEqual({ tolCents: 10, rmsMin: .008, smoothing: .05, wakeLock: false, aiMode: true, bpm: 120, timeSig: 3, subDiv: 'd', refHz: 415, metroVol: .5 })
  })
  test('v1 unknown rms level ignored', () => expect(parseStored(JSON.stringify({ rms: .5 }))).toEqual({}))
  test('v2 round trip', () => {
    const v2 = { v: 2, tolCents: 5, rmsMin: .014, smoothing: .15, wakeLock: true, aiMode: false, bpm: 60, timeSig: 6, subDiv: 1, refHz: 442, metroVol: 1 }
    const { v, ...rest } = v2
